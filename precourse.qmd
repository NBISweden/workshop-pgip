...
to run exercises on Dardel. The PDC account is needed to get access to
the time allocation `{{< var pdc_time_allocation >}}`. This is easiest
done by [applying for a SUPR
account](https://supr.naiss.se/person/register/) and request access to
the PGIP compute project `{{< var naissproject >}}` at
<https://supr.naiss.se/project/> (heading `Request Membership in
Project`). The [PDC support
